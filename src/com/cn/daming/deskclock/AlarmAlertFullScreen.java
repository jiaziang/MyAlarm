...
//----------------------闹钟响的界面----------------------------//
/**
 * 显示在锁屏界面之前 用壁纸作为背景图 Alarm Clock alarm alert: pops visible indicator and plays
 * alarm tone. This activity is the full screen version which shows over the
 * lock screen with the wallpaper as the background.
 */
public class AlarmAlertFullScreen extends Activity {

	// These defaults must match the values in res/xml/settings.xml
	private static final String DEFAULT_SNOOZE = "10"; // 小睡间隔
	private static final String DEFAULT_VOLUME_BEHAVIOR = "2"; // 默认音量
	...
	private void snooze() {
		...
		final long snoozeTime = System.currentTimeMillis() // snooze后的时间
		...
		// Notify the user that the alarm has been snoozed. 通知用户进入snooze模式
	...
	/**
	 * 当前一个闹钟的窗口还在前端时 第二个闹钟响时被触发 this is called when a second alarm is triggered
	 * while a previous alert window is still active.
	 */
	@Override
	...
	protected void onResume() {
		super.onResume();
		// If the alarm was deleted at some point, disable snooze. 如果闹钟被删
		// snooze不可用
