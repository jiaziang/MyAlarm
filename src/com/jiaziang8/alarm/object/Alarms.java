...
public class Alarms {
    ...
    private static ContentValues createContentValues(Alarm alarm) {
        ...
        // A null alert Uri indicates a silent alarm.  alert为空即为静音状态
    ...
    //如果闹钟再下一个小睡模式前响起,清除这个小睡模式，响玲
    private static void clearSnoozeIfNeeded(Context context, long alarmTime) {
        // If this alarm fires before the next snooze, clear the snooze to
        // enable this alarm.
        SharedPreferences prefs =
                context.getSharedPreferences(ViewPagerActivity.PREFERENCES, 0);
     ...
     * content provider.
     * @return Time when the alarm will fire.   返回响铃时间  
     */
    ...
    /**  开机  时间/时区改变  或者用户改变闹钟设置时调用。如果设置snooze则触发，否则加载所有闹钟，触发下一个闹钟。
    ...
    static void saveSnoozeAlert(final Context context, final int id,
        ...
        SharedPreferences prefs = context.getSharedPreferences(
        		ViewPagerActivity.PREFERENCES, 0);
        if (id == -1) {
            clearSnoozePreference(context, prefs);
        } else {
        ...
        // Set the next alert after updating the snooze.  刷新snooze后刷下一个闹钟
        setNextAlert(context);
    }
